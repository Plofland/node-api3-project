...
router.get('/', (req, res) => {
  userFunc
    ...
    .catch(() => {
      res
        .status(500)
        .json({ message: 'Error retrieving the users' });
    });
});
...
router.delete('/:id', validateUserId, (req, res) => {
  ...
  userFunc
    ...
    .catch(() => {
      res
        .status(500)
        .json({ message: 'Error in removing user' });
    });
});
  ...
  userFunc
    ...
    .catch(() => {
      res
        ...
        .json({
          message:
            'The posts from this user could not be retrieved'
        });
    });
});
  ...
  (req, res) => {
    ...
    userFunc
      ...
      .catch(() => {
        res
          .status(500)
          .json({ message: 'Failed to add user post' });
      });
  }
);
